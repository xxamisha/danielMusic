...
// Store your token as an environment variable: VITE_SPOTIFY_TOKEN
...
async function fetchWebApi(endpoint: string, method: string = 'GET', body?: any) {
  if (!TOKEN) {
    throw new Error('Spotify token not found. Set VITE_SPOTIFY_TOKEN in .env');
  }
...
export async function searchAlbums(query: string, limit: number = 20) {
  try {
    const data = await fetchWebApi(
      `v1/search?q=${encodeURIComponent(query)}&type=album&limit=${limit}`
    );
